...
func main() {
	...
	// 2.初始化日志
	if err := logger.Init(settings.Conf.LogConfig, settings.Conf.Model); err != nil {
	...
	if err := snowflake.Init(settings.Conf.StartTime, settings.Conf.MachineID); err != nil {
		panic(fmt.Errorf("init snowflake failed err: %v\n", err))
	}
	// 初始化validator翻译器
	if err := controller.InitTrans("zh"); err != nil {
		panic(fmt.Errorf("init validator failed err: %v\n", err))
	}
